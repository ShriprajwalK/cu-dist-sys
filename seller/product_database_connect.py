...
class ProductDatabaseConnection:
    ...
    def sell_item(self, seller_id, name, category, keywords, condition, price, quantity):
        print("---SELL ITEM db connect")
        print(seller_id, name, category, keywords, condition, price, quantity)
        request = product_service_pb2.SellItemRequest(seller_id=int(seller_id), name=str(name), category=str(category),
                                                      keywords=str(keywords), condition=condition, price=float(price),
                                                      quantity=int(quantity))
        response = self.stub.SellItem(request)
        return response.success

    def update_price(self, item_id, price):
        request = product_service_pb2.UpdatePriceRequest(item_id=int(item_id), price=float(price))
